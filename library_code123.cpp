...
#define CANUSB_INJECT_SLEEP_GAP_DEFAULT 50 /* ms */
...
#define MAX_FRAME_SIZE 13
  ...
  // std::cout << "Data Frame: " << static_cast<unsigned>(data_frame[0]) << " " << static_cast<unsigned>(data_frame[1]) <<"\n";

  if (frame_send(tty_fd, &data_frame[0], data_frame_len) < 0)
  {
...
static int convert_from_hex(const char *hex_string, unsigned char *bin_string, int bin_string_len)
{
 ...
 int inject_data_frame(int tty_fd,  char *hex_id,  char *hex_data)
{
  int data_len;
  ...
  std::cout << "HEX DATA TO SEND: " << hex_data <<  " " << hex_data << "\n";

  unsigned char binary_data[8];
  unsigned char *p3 = &binary_data[0];


  data_len = convert_from_hex(hex_data, p3, sizeof(binary_data));
  if (data_len == 0) {
  ...
  std::cout << "DataLenght: " << data_len << "\n";

  std::cout << "HexID: " << *hex_id <<" String_length: " << strlen(hex_id) << "\n";

  // std::cout << "Hex data: " << (char)*hex_data << (char)*hex_data +1  <<"\n";

  for(int i = 0 ; i < data_len ; i ++ ){
      std:: cout << " / " << hex_data[i] ;//Looping 5 times to print out [0],[1],[2],[3],[4]
}

std::cout << "\n HEX VALUE: "<<hex_value(hex_id[0])*16<< " " << hex_value(hex_id[1])<< "\n";

  switch (strlen(hex_id)) {
  case 1:
    binary_id_lsb = hex_value(hex_id[0]);

    std::cout << binary_id_lsb << " ONE LENGTH binary_LSD" << "\n";
    break;

  case 2:
    binary_id_lsb = hex_value(hex_id[0]) * 16 + hex_value(hex_id[1]);

    std::cout <<   static_cast<unsigned>(binary_id_lsb) <<" "<< (unsigned char)binary_id_lsb << " TWO LENGTH binary_LSD" << "\n";
    break;
  ...
  std::cout << "Binary_id (msb): " << static_cast<unsigned>(binary_id_msb)  <<" " << binary_id_msb << "\n";
  std::cout << "Binary_id (lsb): " << static_cast<unsigned>(binary_id_lsb) <<  "\n";
...
static void dump_data_frames(int tty_fd)
{
 ...
 int adapter_init( char *device, int baudrate)
{
  int tty_fd, result;
  struct termios2 tio;
...
//   CANUSB_SPEED speed = CANUSB_SPEED_5000;
